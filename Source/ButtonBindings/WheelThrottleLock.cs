...
namespace AdvancedInput.ButtonBindings {
	...
	public class AI_BB_WheelThrottleLock: IButtonBinding
	{
		...
		public void Update (int state, bool edge)
		{
			switch (mode) {
				case ButtonMode.hold:
					flightControl.wheelThrottleLock = state != 0;
					break;
				case ButtonMode.toggle:
					if (state > 0 && edge) {
						flightControl.wheelThrottleLock ^= true;
					}
					break;
				case ButtonMode.off:
					if (state > 0 && edge) {
						flightControl.wheelThrottleLock = false;
					}
					break;
				case ButtonMode.on:
					if (state > 0 && edge) {
		...
		public AI_BB_WheelThrottleLock (AI_FlightControl fc, ConfigNode node)
		{
			flightControl = fc;
			if (node != null) {
